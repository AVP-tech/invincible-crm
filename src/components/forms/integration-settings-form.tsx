...
const integrationCardClass =
  "space-y-4 rounded-4xl border border-black/5 bg-white p-6 text-slate-950 shadow-[0_20px_60px_rgba(15,23,42,0.08)]";
const sectionTitleClass = "text-lg font-semibold text-slate-950";
const sectionBodyClass = "mt-1 text-sm text-slate-700";
const sectionMetaClass = "text-sm text-slate-700";
const sectionHintClass = "text-xs text-slate-700";
const fieldSurfaceClass =
  "text-slate-950 placeholder:text-slate-400 dark:!border-black/10 dark:!bg-white dark:!text-slate-950 dark:placeholder:!text-slate-400";
...
export function IntegrationSettingsForm({
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      <form className={integrationCardClass} onSubmit={saveEmail}>
        <div>
          <p className={sectionTitleClass}>Email sync</p>
          <p className={sectionBodyClass}>Connect a shared mailbox over IMAP and pull fresh email context into the CRM.</p>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Connection name">
            <Input value={emailName} onChange={(event) => setEmailName(event.target.value)} disabled={!canManage} className={fieldSurfaceClass} />
          </Field>
          <Field label="Mailbox">
            <Input value={mailbox} onChange={(event) => setMailbox(event.target.value)} disabled={!canManage} className={fieldSurfaceClass} />
          </Field>
          <Field label="IMAP host">
            <Input value={host} onChange={(event) => setHost(event.target.value)} disabled={!canManage} placeholder="imap.gmail.com" className={fieldSurfaceClass} />
          </Field>
          <Field label="Port">
            <Input type="number" value={port} onChange={(event) => setPort(event.target.value)} disabled={!canManage} className={fieldSurfaceClass} />
          </Field>
          <Field label="Username">
            <Input value={username} onChange={(event) => setUsername(event.target.value)} disabled={!canManage} className={fieldSurfaceClass} />
          </Field>
        ...
        <p className={sectionHintClass}>Saving now verifies the mailbox before it is marked connected.</p>
        <div className={`space-y-1 ${sectionMetaClass}`}>
        ...
        {emailDefaults?.lastSyncMessage ? <p className={sectionMetaClass}>{emailDefaults.lastSyncMessage}</p> : null}
      </form>

      <form className={integrationCardClass} onSubmit={saveWhatsapp}>
        <div>
          <p className={sectionTitleClass}>WhatsApp sync</p>
          <p className={sectionBodyClass}>Use the Meta webhook path below to ingest incoming WhatsApp messages into the CRM automatically.</p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Connection name">
            <Input value={whatsappName} onChange={(event) => setWhatsappName(event.target.value)} disabled={!canManage} className={fieldSurfaceClass} />
          </Field>
          <Field label="Phone number id">
            <Input value={phoneNumberId} onChange={(event) => setPhoneNumberId(event.target.value)} disabled={!canManage} className={fieldSurfaceClass} />
          </Field>
          <Field label="Verify token">
            <Input value={verifyToken} onChange={(event) => setVerifyToken(event.target.value)} disabled={!canManage} className={fieldSurfaceClass} />
          </Field>
        ...
        <Field label="Webhook URL">
          <Textarea value={webhookPath} readOnly className={`min-h-[96px] ${fieldSurfaceClass}`} />
        </Field>
        <div className="flex flex-wrap gap-3">
        ...
        <p className={sectionHintClass}>Paste this exact URL into your Meta app webhook settings, then subscribe the WhatsApp message events.</p>
        <div className={`space-y-1 ${sectionMetaClass}`}>
        ...
        {whatsappDefaults?.lastSyncMessage ? <p className={sectionMetaClass}>{whatsappDefaults.lastSyncMessage}</p> : null}
      </form>
    </div>
  );
}
