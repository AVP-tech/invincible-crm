...
import { ArrowRight, BrainCircuit, CheckCircle2, Crown, KanbanSquare, type LucideIcon } from "lucide-react";
  ...
  return (
    <div className="cinematic-auth-shell min-h-screen">
      <div className="pointer-events-none absolute inset-0">
        <div className="cinematic-orb left-[-8rem] top-[-6rem] h-[22rem] w-[22rem] bg-[#3d2a08]" />
        <div className="cinematic-orb bottom-[-8rem] right-[-8rem] h-[24rem] w-[24rem] bg-[#325b45]" />
        <div className="cinematic-orb left-1/2 top-1/3 h-[16rem] w-[16rem] -translate-x-1/2 bg-[#7f4b21] opacity-20" />
      ...
      <div className="relative mx-auto flex min-h-screen max-w-7xl items-center px-4 py-8 lg:px-8">
        <div className="grid w-full gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="cinematic-auth-panel flex flex-col justify-between rounded-[2.3rem] p-8 text-white lg:p-12">
            <div>
              <Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-gold/60 transition hover:text-gold">
                <Crown className="h-4 w-4" />
                Invincible CRM
                <ArrowRight className="h-4 w-4" />
              </Link>
              <p className="mt-8 text-xs font-semibold uppercase tracking-[0.35em] text-gold/40">AI-powered business OS</p>
              <h1 className="mt-5 max-w-xl font-serif text-4xl leading-tight lg:text-6xl">
                Calm enough for non-technical teams. Sharp enough to feel <span className="gradient-text-gold">invincible</span>.
              </h1>
              <p className="mt-6 max-w-2xl text-base leading-8 text-white/68">
                Invincible CRM is built for founders, consultants, and micro teams who want the system to move in the
            ...
            <div className="mt-10 grid gap-4">
              {proofPoints.map((point, index) => (
                <div
                  key={point.title}
                  className="cinematic-auth-float rounded-[1.75rem] border border-gold/10 bg-gold/[0.04] p-5"
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {featureIcons.map(({ icon: Icon, label }) => (
                <div key={label} className="rounded-[1.55rem] border border-gold/10 bg-black/20 px-4 py-4 text-white/75 backdrop-blur-md">
